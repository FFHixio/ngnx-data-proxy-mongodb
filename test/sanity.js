...
test('Expand nested objects to independent collections', function (t) {
  ...
  clearDB(() => {
    p.proxy.save(() => {
      ...
      p.proxy.fetch(() => {
        console.log('HIT FETCH CALLBACK')
        t.ok(p.firstname === 'The' && p.lastname === 'Doctor', 'Primary data object retrieved successfully.')
        t.ok(p.pets.length === 2, 'Correct number of nested objects detected.')
        t.ok(p.pets.last.toys.length === '2', 'Multiple layers of nesting retrieved from multiple collections.')
...
//
//       tasks.on('complete', () => {
//         record.proxy.once('disconnected', () => {
//           t.end()
//         })
//
